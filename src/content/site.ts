...
export const process = [
  ...
  [
    '04',
    'Работаем',
    'Пробуем новые стратегии и постепенно передаем ребенку ответственность.'
  ],
  [
    '05',
    'Снижаем поддержку',
    'Когда самостоятельности становится больше, объем сопровождения уменьшается.'
  ]
] as const;
...
export const offers = [
  {
    ...
    details: '3 встречи с ребенком + 1 встреча с родителями + письменный отчет.',
...
export const faq = [
  ...
  [
    ...
    'Основной фокус — не репетиторство, а то, как ребенок учится: самостоятельность, самоорганизация, мотивация и стратегии. Конкретная задача может стать материалом для работы над навыками.'
  ],
  [
    'А если ребенок сам не хочет заниматься?',
    'Это нормально. Работа не строится на принуждении: сначала важно понять, что происходит и почему ребенок сопротивляется.'
  ],
  [
    'Родители присутствуют на занятиях?',
    'Как правило, нет. Ребенку нужно собственное пространство, а родители получают необходимую обратную связь по процессу и динамике.'
  ],
  [
