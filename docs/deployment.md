...
Если SMTP временно недоступен, заявка остается в базе со статусом `failed`, а при аварийном завершении процесса может остаться в `pending`. Системный cron на VDS повторяет отправку записей `failed` сразу, а записей `pending` — когда они старше 10 минут, чтобы не конкурировать с ограниченной по времени SMTP-попыткой текущего запроса. Добавьте отдельную строку в root crontab:
...
Настройте мониторинг сообщений `nina-notification-retry` в системном журнале как alert. Скрипт обрабатывает до 20 самых старых подходящих записей, после успешной отправки меняет статус на `sent`, переводит неотправленную устаревшую запись `pending` в `failed`, продолжает пакет после отдельной ошибки и возвращает ненулевой код при ошибке отправки или записи статуса. В cron-журнал попадают только ID заявок и счетчики статусов, без имени, контакта и текста обращения. Периодически проверяйте число и ID записей `failed` и устаревших `pending`; их содержимое не выводите в журнал.

Порог для `pending` можно изменить только для retry-скрипта необязательной переменной `NOTIFICATION_PENDING_AFTER_MINUTES` со значением от `1` до `1440`; если переменная отсутствует, используются консервативные 10 минут. Без подтвержденной необходимости оставьте значение по умолчанию.

Доставка имеет семантику at-least-once: если API или retry-процесс завершится после принятия письма SMTP-сервером, но до записи статуса `sent` в SQLite, устаревшая `pending` или `failed` заявка может быть отправлена повторно. ID заявки в теме позволяет распознать такой дубликат. Reconciliation предотвращает постоянный пропуск уведомления, но SMTP не позволяет гарантировать exactly-once доставку.
...
Убедитесь, что записи `failed` и синтетическая `pending` старше 10 минут перешли в `sent`, свежая `pending` осталась без изменений, а повторный запуск сообщает нулевое число записей. Отдельно убедитесь, что ошибка SMTP переводит устаревшую `pending` в `failed` и возвращает ненулевой код. Не используйте Gmail для этой проверки. Ежедневный backup остается отдельной обязательной задачей cron.
...
- Host cron повторяет SMTP-уведомления; записи `failed` и `pending` старше 10 минут переходят в `sent`, свежая `pending` остается нетронутой, повторный запуск обрабатывает ноль записей, а ненулевой код вызывает alert.
